...
mm <- na.omit(mm[,c('cohort_year', 'meancond', 'Capt1', 'ArcCodt1', 'ice.1y.jan', 'lcc', 'ucc', 'ncond')])
...
m7 <- gam(meancond ~ s(ArcCodt1)  + s(ice.1y.jan, k = 5), data = mm)
m8 <- gam(meancond ~ s(Capt1)  + s(ice.1y.jan, k = 5), data = mm)
# m9 <- gam(meancond ~ te(Capt1, ice.1y.jan, bs = 'tp', m = 1), data = mm)
# m10 <- gam(meancond ~ s(log(capbiomass))  + s(ice.1y.jan, k = 5), data = mm)
# m11 <- gam(meancond ~ s(log(capbiomass)), data = mm)
...
abgam12 <-   expression(paste("condition ~ gam(smooth(Mid-winter ice), R"[italic(adj)]^2,"=",0.42))
abgam8 <-   expression(paste("condition ~ gam(smooth(Mid-winter ice) + smooth(Capelin)), R"[italic(adj)]^2,"=",0.44))
...
ay <- ay + theme_set(theme_cowplot())
ay <- ay + labs(x = 'Year', y = 'Condition')
ay <- ay + annotate("text", x = 1997, y = 0.65, label = abgam12, hjust = 0, size = 2)
ay <- ay + annotate("text", x = 1997, y = 0.6, label = abgam8, hjust = 0, size = 2)
ay <- ay + annotate("segment", x = 1996, xend = 1996.8, y = 0.65, yend = 0.65, colour = mypalette[1])
